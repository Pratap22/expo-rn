  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Paragraph>Total Expenses: ${expenses.reduce((sum, expense) => sum + expense.amount, 0).toFixed(2)}</Paragraph>
        </Card.Content>
      </Card>
      ...
      {settlements.map((settlement, index) => (
        <Card key={index} style={styles.card}>
          <Card.Content>
            <Paragraph>{`${settlement.from} owes ${settlement.to}: $${settlement.amount.toFixed(2)}`}</Paragraph>
          </Card.Content>
        </Card>
      ))}
    </ScrollView>
